...
1. **v0 — каркас даних.** Додати підтримку `week.json` у `js/app.js` (перекриття
   поточного тижня + бейдж «адаптовано»). Сторінка готова приймати згенеровані тижні
   ще до будь-яких інтеграцій.
2. **v1 — Strava-ритуал (легально, без Garmin).** Ручний щотижневий Claude-ритуал:
   офіційний Strava MCP → аналіз навантаження → `week.json`. Уже корисно.
3. **v2 — додати Garmin (recovery).** Підключити локальний неофіційний Garmin MCP,
   додати Training Readiness / HRV / сон у логіку вердикту. Тут з'являється «реалізм».
